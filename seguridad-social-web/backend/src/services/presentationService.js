...
async function resolvePresentationRequest(presentationRequestUrl) {
    ...
    const config = {
        headers: { Authorization: `Bearer ${token}`, Accept: 'text/plain', 'Content-Type': 'text/plain' }
    };
...
async function getOrSelectDidSomewhere() {
    ...
    if (!token || !walletId) {
        throw new Error('[getOrSelectDidSomewhere] No se encontró token o walletId');
    }

    // 3) listar DIDs en la wallet
    const dids = await listDIDs(token, walletId);
    if (!dids || !dids.length) {
        throw new Error('[getOrSelectDidSomewhere] No se encontraron DIDs en la wallet');
    }
