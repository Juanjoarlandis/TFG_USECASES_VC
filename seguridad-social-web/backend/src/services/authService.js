...
module.exports = {
  ...
  async loginAndVerifyIdentityCredential(email, password) {
    ...
    logger.debug(
      "[authService] HolderSessionManager.loginHolderWithCredentials OK.",
    );
    ...
    logger.debug(
      `[authService] userInfo: ${JSON.stringify(userInfo, null, 2)}`,
    );
    ...
    logger.debug(
      `[authService] resolvedPresentationRequest = ${JSON.stringify(resolvedPresentationRequest, null, 2)}`,
    );
    ...
    logger.debug(
      `[authService] presentationDefinition: ${JSON.stringify(presentationDefinition, null, 2)}`,
    );
    ...
    logger.debug(
      `[authService] usePresentationRequest response => ${JSON.stringify(useResp, null, 2)}`,
    );
