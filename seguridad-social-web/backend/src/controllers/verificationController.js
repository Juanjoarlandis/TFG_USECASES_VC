...
module.exports = {
  ...
  async offerVerification(req, res, next) {
    try {
      ...
      const result = await verificationService.offerVerificationOneCred(
        req.body,
      );
      ...
      logger.error(
        "[verificationController] Error en offerVerification:",
        error.message,
      );
      if (error.status) {
  ...
  async offerVerification3CredsAutomatic(req, res, next) {
    try {
      logger.debug(
        "[verificationController] offerVerification3CredsAutomatic - start",
      );
      const result =
        await verificationService.offerVerification3CredsAutomatic();
      // result => { message, state, verificationUrl }
      return res.status(200).json(result);
    } catch (error) {
      logger.error(
        "[verificationController] Error en offerVerification3CredsAutomatic:",
        error.message,
      );
      if (error.status) {
        return res.status(error.status).json({ error: error.message });
  ...
  async offerVerification3Creds(req, res, next) {
    try {
      ...
      logger.error(
        "[verificationController] Error en offerVerification3Creds:",
        error.message,
      );
      if (error.status) {
        return res.status(error.status).json({ error: error.message });
  ...
  async statusCallbackAlta(req, res, next) {
    try {
      ...
      await verificationService.handleStatusCallbackAlta(
        stateId,
        verificationData,
      );
      return res
        .status(200)
        .send("Status callback alta processed successfully");
    } catch (error) {
      logger.error(
        "[verificationController] Error en statusCallbackAlta:",
        error.message,
      );
      if (error.status) {
        return res.status(error.status).json({ error: error.message });
  ...
  async statusCallback(req, res, next) {
    try {
      ...
      return res
        .status(200)
        .json({ message: "statusCallback processed successfully" });
    } catch (error) {
      logger.error(
        "[verificationController] Error en statusCallback:",
        error.message,
      );
      if (error.status) {
        return res.status(error.status).json({ error: error.message });
  ...
  async statusCallbackWalletLogin(req, res, next) {
    try {
      logger.debug(
        "[verificationController] statusCallbackWalletLogin - start",
      );
      const { stateId } = req.params;
      await verificationService.handleStatusCallbackWalletLogin(
        stateId,
        req.body,
      );
      ...
      logger.error(
        "[verificationController] Error en statusCallbackWalletLogin:",
        error.message,
      );
      if (error.status) {
        return res.status(error.status).json({ error: error.message });
  ...
  async getVerificationSession(req, res, next) {
    try {
      ...
      const { stateId } = req.params;
      const sessionInfo =
        await verificationService.getVerificationSession(stateId);
      return res.status(200).json(sessionInfo);
    } catch (error) {
      logger.error(
        "[verificationController] Error en getVerificationSession:",
        error.message,
      );
      if (error.status) {
        return res.status(error.status).json({ error: error.message });
