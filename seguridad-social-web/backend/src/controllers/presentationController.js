...
module.exports = {
    async resolvePresentationRequest(req, res, next) {
        try {
            ...
            logger.error('[presentationController] Error resolving presentation request:', error.message);
            if (error.status) {
    ...
    async matchCredentialsForPresentation(req, res, next) {
        try {
            ...
            logger.debug(`[presentationController] matchCredentialsForPresentation => ${JSON.stringify(presentationDefinition)}`);
            const data = await matchCredentialsForPresentation(presentationDefinition);
            logger.debug(`[presentationController] data => ${JSON.stringify(data, null, 2)}`);
            return res.status(200).json(data);
        } catch (error) {
            logger.error('[presentationController] Error matching credentials:', error.message);
            if (error.status) {
                return res.status(error.status).json({ error: error.message });
            }
            next(error);
    ...
    async usePresentationRequest(req, res, next) {
        try {
            ...
            logger.error('[presentationController] Error using presentation request:', error.message);
            if (error.status) {
                return res.status(error.status).json({ error: error.message });
            }
            next(error);
