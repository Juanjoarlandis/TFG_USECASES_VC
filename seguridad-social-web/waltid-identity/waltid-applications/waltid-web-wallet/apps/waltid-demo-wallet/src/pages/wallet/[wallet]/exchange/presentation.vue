<template>
  <div>
    <CenterMain>
      ...
      <div v-else class="my-10 mb-40 sm:mb-10 overflow-scroll">
        ...
        <div class="w-full flex justify-center gap-5" v-else>
          <button
            v-if="matchedCredentials.length > 1"
            @click="index--"
          ...
          <div v-for="credential in matchedCredentials" :key="credential.id">
            <div v-if="credential.disclosures && selection[credential.id]">
              <div class="sm:flex justify-between items-center">
                <div
                  @click="toggleDisclosure(credential.id)"
                  :class="{
                    'font-semibold': disclosureModalState[credential.id],
                  }"
                ...
                >
                  {{
                    (
                      ...
                      parseJwt(credential.document).vc ??
                  ...
                  <svg
                    v-if="disclosureModalState[credential.id]"
                    width="16"
                    height="16"
                    viewBox="0 0 17 10"
                    fill="none"
                ...
                <div>
                  ...
                  >
                    ...
                    <input
                      ...
                      @click="
                        disclosures[credential.id] = $event.target?.checked
                          ? parseDisclosures(credential.disclosures as string)
                          : []
                      "
                      :checked="
                        disclosures[credential.id]?.length ===
                        parseDisclosures(credential.disclosures)?.length
                      "
                    />
                  </div>
                  <div v-else>
                  ...
                  >
                    ...
                    <div class="ml-3 flex h-6 items-center">
                      <input
                        ...
                        type="checkbox"
                        :checked="
                          disclosures[credential.id] &&
                          disclosures[credential.id].find(
                            (elem: Array<string>) => elem[0] === disclosure[0],
                          )
                        "
                        @click="
    ...
    <div
      v-if="!failed && matchedCredentials.length"
      class="w-full sm:max-w-2xl sm:mx-auto"
    >
      <div
